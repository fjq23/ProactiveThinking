...
ENV_NAME="${ENV_NAME:?ENV_NAME is required (20q or clinlic)}"
METHOD="${METHOD:?METHOD is required}"

case "$ENV_NAME" in
  20q)
    WORK_DIR="$ROOT_DIR/env/20q"
    DEFAULT_INPUT="$ROOT_DIR/env/20q/data/test_20q.json"
    ;;
  clinlic)
    WORK_DIR="$ROOT_DIR/env/clinlic"
    DEFAULT_INPUT="$ROOT_DIR/env/clinlic/data/agentclinic_medqa_extended.jsonl"
    ;;
  *)
...
OUTPUT_FILE="${OUTPUT_FILE:-$OUTPUT_DIR/${ENV_NAME}_${METHOD}.jsonl}"
SUMMARY_FILE="${SUMMARY_FILE:-$OUTPUT_DIR/${ENV_NAME}_${METHOD}.txt}"
